  ...
  const exportDebugReport = async () => {
    try {
      ...
      const report = [
        ...
        `批量进度: ${
          stats.batchProgress ? JSON.stringify(stats.batchProgress) : '无'
        }`,
        ``,
      ...
      const reportPath = await path.join(
        downloadDir,
        'x-spider-debug-report.txt',
      );
  ...
  return (
    <>
      ...
      <Section title="网络代理" name="proxy" titleIcon={<GlobalOutlined />}>
        ...
        <Item label="代理地址" settingKey="url">
      ...
      <Section title="应用常规" name="app">
        ...
        <Space>
          ...
          <Button icon={<BugOutlined />} onClick={exportDebugReport}>
            导出诊断报告
          </Button>
        </Space>
      </Section>
